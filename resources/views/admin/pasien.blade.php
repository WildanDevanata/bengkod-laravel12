...
@section('content')
    <section class="content py-8 bg-gray-50 min-h-screen">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl shadow-xl overflow-hidden animate-fade-in">
                <div class="p-6 bg-gradient-to-r from-blue-600 to-blue-700 text-white flex justify-between items-center">
                    <h3 class="text-2xl font-bold tracking-tight">
                        <i class="fas fa-user-injured mr-2"></i> Daftar Pasien
                    </h3>
                    <button type="button" class="inline-flex items-center px-4 py-2 bg-blue-500 text-white rounded-lg shadow-md hover:bg-blue-600 hover:scale-105 transition transform duration-200" data-toggle="modal" data-target="#addPasienModal">
                ...
                <div class="p-6 sm:p-8">
                    ...
                    <div class="overflow-x-auto">
                        <table id="pasienTable" class="w-full text-left border-collapse">
                            <thead class="bg-gray-100 text-gray-800">
                                <tr>
                                    <th class="p-4 text-sm font-semibold w-12">#</th>
                                    <th class="p-4 text-sm font-semibold">No RM</th>
                            ...
                            <tbody>
                                @forelse($users as $index => $pasien)
                                    <tr class="border-b hover:bg-gray-50 transition duration-200">
                                        ...
                                        <td class="p-4 text-sm">{{ $pasien->no_rm }}</td>
                                        <td class="p-4 text-sm">{{ $pasien->no_ktp }}</td>
                                        <td class="p-4 text-sm font-semibold">{{ $pasien->name }}</td>
                                        <td class="p-4 text-sm">{{ $pasien->email }}</td>
                                        <td class="p-4 text-sm">{{ $pasien->no_hp ?? '-' }}</td>
                                        <td class="p-4 text-sm">{{ $pasien->alamat ?? '-' }}</td>
                                    ...
                                    <tr>
                                        <td colspan="8" class="text-center py-12">
                                            <div class="animate-fade-in">
                                                <i class="fas fa-user-injured text-6xl text-gray-300 mb-4 animate-pulse"></i>
            ...
            <!-- Modal Add Pasien -->
            <div class="modal fade" id="addPasienModal" tabindex="-1" role="dialog">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        ...
                                        <input type="text" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('no_ktp') border-red-500 @enderror"
                                               id="no_ktp" name="no_ktp" value="{{ old('no_ktp') }}" placeholder="Masukkan NIK pasien" required>
                                        @error('no_ktp')
                                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    <div>
                                        <label for="name" class="block text-sm font-semibold text-gray-700">Nama Pasien <span class="text-red-500">*</span></label>
                                        <input type="text" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('name') border-red-500 @enderror"
                                               id="name" name="name" value="{{ old('name') }}" placeholder="Masukkan nama lengkap pasien" required>
                                        @error('name')
                                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    <div>
                                        ...
                                        <input type="email" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('email') border-red-500 @enderror"
                                               id="email" name="email" value="{{ old('email') }}" placeholder="pasien@example.com" required>
                                        @error('email')
                                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                        @enderror
                                    </div>
